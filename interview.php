                       ...
                       $sql = "SELECT `id`, `video_link`, `details`, `description`, `category`, `uploadFile`, `status`, `upload_date` FROM `interview` where `status` ='1'ORDER BY id DESC";//(innerjoin)
                        $result = mysqli_query($conn, $sql);
                         ...
                         while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            <a href='interviews_view.php?id=" . $row['id'] . "'?id=" . $row['id'] . "'>
                            <div class='utf_post_block_style clearfix'>
                           <div class='utf_post_thumb utf_post_img'> 
                           <img class='img-fluid inter-img' src='./admin/interview_docs/".$row['id']."/photo/".$row['uploadFile']."'></div>
                                <div class='utf_post_cat'>" . $row['category'] ."</div>
                                <div class='utf_post_content'>
                                    <h2 class='utf_post_title title-medium dottss'> " . $row['details'] ."</h2>
                                    <div class='utf_post_meta'>  <span class='utf_post_date'><i class='fa fa-calendar'></i>". date('l d M, Y',strtotime($row['upload_date']))."</span> </div></div>
                                    ​</a>  </div>
                            </a>
                        </a>
                    </div>
